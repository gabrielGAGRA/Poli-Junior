...
web_search_preview = WebSearchTool(
    search_context_size="medium", user_location={"type": "approximate"}
)
file_search = FileSearchTool(vector_store_ids=["vs_68d1adb002b481918d197bbe50ee1974"])


class RedatorDeNurturingCaseSchema(BaseModel):
    titulo: str
    corpo_html: str


class RedatorDeRetomadaFupSchema(BaseModel):
    titulo: str
    corpo_html: str


class RedatorDeReEngajementPSNurturingFupSchema(BaseModel):
    titulo: str
    corpo_html: str


class RedatorDeRetomadaCasePesquisaSchema(BaseModel):
    titulo: str
    corpo_html: str


class RedatorDeNurturingPesquisaSchema(BaseModel):
    titulo: str
    corpo_html: str


class PesquisadorContext:
    def __init__(self, workflow_input_as_text: str, state_emails_anteriores: str):
        self.workflow_input_as_text = workflow_input_as_text
        self.state_emails_anteriores = state_emails_anteriores


def pesquisador_instructions(
    run_context: RunContextWrapper[PesquisadorContext],
    _agent: Agent[PesquisadorContext],
):
    workflow_input_as_text = run_context.context.workflow_input_as_text
    state_emails_anteriores = run_context.context.state_emails_anteriores
    return f"""Você é um Agente de IA especialista em Inteligência de Mercado e Pesquisa para vendas B2B, atuando como um analista para o Núcleo de Dados & IA da Poli Júnior. A sua tarefa é executar uma instrução de pesquisa específica, baseada no contexto de um negócio, e entregar um dossiê de inteligência estruturado.
...
**CONTEXTO DO NEGÓCIO:** {workflow_input_as_text}
**CONTEXTO DE E-MAILS ANTERIORES:** {state_emails_anteriores}
...
pesquisador = Agent(
    name="Pesquisador",
    instructions=pesquisador_instructions,
    model="gpt-5.4",
...
A sua resposta DEVE ser um objeto JSON válido como abaixo:
{
  ...
  \"corpo_html\": \"O corpo do e-mail em HTML, escrito de acordo com a sua persona e as regras. NUNCA inclua referências técnicas.\"
    ...
    model="gpt-4.1",
    tools=[file_search],
    output_type=RedatorDeNurturingCaseSchema,
    model_settings=ModelSettings(
        temperature=0.8, top_p=1, max_tokens=10000, store=True
    ),
)


redator_de_retomada_fup = Agent(
    name="Redator de Retomada - FUP",
    instructions="""Você é um Agente de IA especialista em Redação para Reativação de Oportunidades (Retomada) do **Núcleo de ANÁLISE DE DADOS E INTELIGÊNCIA ARTIFICIAL da Poli Júnior**. A sua persona é a de um consultor sênior, direto e focado em resultados. A sua missão é executar uma \"intervenção cirúrgica\": uma cadência curta e intensa para requalificar uma oportunidade que está \"fria\" há 3-6 meses.
...
A sua resposta DEVE ser um objeto JSON válido como abaixo:
{
    ...
    model="gpt-4.1",
    output_type=RedatorDeRetomadaFupSchema,
    model_settings=ModelSettings(
        temperature=0.8, top_p=1, max_tokens=10000, store=True
    ),
)
...
A sua resposta DEVE ser um objeto JSON válido como abaixo:
{
  ...
  \"corpo_html\": \"O corpo do e-mail em HTML, escrito de acordo com a sua persona e as regras desta cadência final.\"
    ...
    model="gpt-4.1",
    output_type=RedatorDeReEngajementPSNurturingFupSchema,
    model_settings=ModelSettings(
        temperature=0.8, top_p=1, max_tokens=10000, store=True
    ),
)
...
redator_de_retomada_case_pesquisa = Agent(
...
A sua resposta DEVE ser um objeto JSON válido como abaixo:
{
    ...
    model="gpt-5.4",
    tools=[file_search],
    output_type=RedatorDeRetomadaCasePesquisaSchema,
    model_settings=ModelSettings(store=True, reasoning=Reasoning(effort="medium")),
)


redator_de_nurturing_pesquisa = Agent(
...
A sua resposta DEVE ser um objeto JSON válido como abaixo:
{
  ...
  \"corpo_html\": \"O corpo do e-mail em HTML, escrito de acordo com a sua persona e as regras. NUNCA inclua referências técnicas.\"
    ...
    model="gpt-5.4",
    output_type=RedatorDeNurturingPesquisaSchema,
    model_settings=ModelSettings(store=True, reasoning=Reasoning(effort="medium")),
)


class PesquisadorContext1:
    def __init__(self, workflow_input_as_text: str, state_emails_anteriores: str):
        self.workflow_input_as_text = workflow_input_as_text
        self.state_emails_anteriores = state_emails_anteriores


def pesquisador_instructions1(
    run_context: RunContextWrapper[PesquisadorContext1],
    _agent: Agent[PesquisadorContext1],
):
    workflow_input_as_text = run_context.context.workflow_input_as_text
    state_emails_anteriores = run_context.context.state_emails_anteriores
    return f"""Você é um Agente de IA especialista em Inteligência de Mercado e Pesquisa para vendas B2B, atuando como um analista para o Núcleo de Dados & IA da Poli Júnior. A sua tarefa é executar uma instrução de pesquisa específica, baseada no contexto de um negócio, e entregar um dossiê de inteligência estruturado.
...
pesquisador1 = Agent(
    name="Pesquisador",
    instructions=pesquisador_instructions1,
    model="gpt-5.4",
    tools=[web_search_preview],
    model_settings=ModelSettings(store=True, reasoning=Reasoning(effort="low")),
)
...
async def run_workflow(workflow_input: WorkflowInput):
    with trace("Fluxo NDados"):

        def _fallback_output(reason: str) -> dict[str, str]:
            return {
                "titulo": "Contexto insuficiente para gerar e-mail",
                "corpo_html": (
                    "<p>Nao foi possivel executar o fluxo com os dados enviados.</p>"
                    f"<p>Motivo: {reason}</p>"
                ),
            }
        ...
        state = {
            "cadencia": workflow.get("cadencia"),
            "etapa": workflow.get("etapa"),
            "emails_anteriores": workflow.get("emails_anteriores") or "",
        }
        ...
        if state["cadencia"] == "Nurturing":
            if state["etapa"] == 1 or state["etapa"] == 3 or state["etapa"] == 5:
                pesquisador_result_temp = await Runner.run(
                    pesquisador,
                    input=[*conversation_history],
                    run_config=RunConfig(
                        trace_metadata={
                            "__trace_source__": "agent-builder",
                            "workflow_id": "wf_69a712cef21c8190bcc1c573a9feaad40c5ca413b5fe04d2",
                        }
                    ),
                    context=PesquisadorContext(
                        workflow_input_as_text=workflow["input_as_text"],
                        state_emails_anteriores=state["emails_anteriores"],
                    ),
                )

                conversation_history.extend(
                    [item.to_input_item() for item in pesquisador_result_temp.new_items]
                )

                pesquisador_result = {
                    "output_text": pesquisador_result_temp.final_output_as(str)
                }
                redator_de_nurturing_pesquisa_result_temp = await Runner.run(
                    redator_de_nurturing_pesquisa,
                    input=[
                ...
                conversation_history.extend(
                    [
                        item.to_input_item()
                        for item in redator_de_nurturing_pesquisa_result_temp.new_items
                    ]
                )

                redator_de_nurturing_pesquisa_result = {
                    "output_text": redator_de_nurturing_pesquisa_result_temp.final_output.json(),
                    "output_parsed": redator_de_nurturing_pesquisa_result_temp.final_output.model_dump(),
                }
                end_result = {
                    "titulo": redator_de_nurturing_pesquisa_result["output_parsed"][
                        "titulo"
                    ],
                    "corpo_html": redator_de_nurturing_pesquisa_result["output_parsed"][
                        "corpo_html"
                    ],
                }
                return end_result
            elif state["etapa"] == 2 or state["etapa"] == 4:
                redator_de_nurturing_case_result_temp = await Runner.run(
                    redator_de_nurturing_case,
                    input=[
                        *conversation_history,
                        {
                            "role": "user",
                ...
                conversation_history.extend(
                    [
                        item.to_input_item()
                        for item in redator_de_nurturing_case_result_temp.new_items
                    ]
                )

                redator_de_nurturing_case_result = {
                    "output_text": redator_de_nurturing_case_result_temp.final_output.json(),
                    "output_parsed": redator_de_nurturing_case_result_temp.final_output.model_dump(),
                }
                end_result = {
                    "titulo": redator_de_nurturing_case_result["output_parsed"][
                        "titulo"
                    ],
                    "corpo_html": redator_de_nurturing_case_result["output_parsed"][
                        "corpo_html"
                    ],
                }
                return end_result
            else:
                return _fallback_output("Etapa de Nurturing fora do esperado.")
        elif state["cadencia"] == "Retomada":
            if state["etapa"] == 1:
                pesquisador_result_temp = await Runner.run(
                    pesquisador1,
                    input=[*conversation_history],
                ...
                redator_de_retomada_case_pesquisa_result_temp = await Runner.run(
                    redator_de_retomada_case_pesquisa,
                    input=[
                        *conversation_history,
                        {
                            "role": "user",
                            "content": [
                                {
                                    "type": "input_text",
                                    "text": f"""CONTEXTO DO NEGÓCIO: {workflow["input_as_text"]}
                PESQUISA: {pesquisador_result["output_text"]}
                ...
                conversation_history.extend(
                    [
                        item.to_input_item()
                        for item in redator_de_retomada_case_pesquisa_result_temp.new_items
                    ]
                )

                redator_de_retomada_case_pesquisa_result = {
                    "output_text": redator_de_retomada_case_pesquisa_result_temp.final_output.json(),
                    "output_parsed": redator_de_retomada_case_pesquisa_result_temp.final_output.model_dump(),
                }
                end_result = {
                    "titulo": redator_de_retomada_case_pesquisa_result["output_parsed"][
                        "titulo"
                    ],
                    "corpo_html": redator_de_retomada_case_pesquisa_result[
                        "output_parsed"
                    ]["corpo_html"],
                }
                return end_result
            elif state["etapa"] == 2 or state["etapa"] == 3 or state["etapa"] == 4:
                redator_de_retomada_fup_result_temp = await Runner.run(
                    redator_de_retomada_fup,
                    input=[
                        *conversation_history,
                        {
                            "role": "user",
                ...
                conversation_history.extend(
                    [
                        item.to_input_item()
                        for item in redator_de_retomada_fup_result_temp.new_items
                    ]
                )

                redator_de_retomada_fup_result = {
                    "output_text": redator_de_retomada_fup_result_temp.final_output.json(),
                    "output_parsed": redator_de_retomada_fup_result_temp.final_output.model_dump(),
                }
                end_result = {
                    "titulo": redator_de_retomada_fup_result["output_parsed"]["titulo"],
                    "corpo_html": redator_de_retomada_fup_result["output_parsed"][
                        "corpo_html"
                    ],
                }
                return end_result
            else:
                return _fallback_output("Etapa de Retomada fora do esperado.")
        elif state["cadencia"] == "Re-engajement do Nurturing":
            redator_de_re_engajement_p_s_nurturing_fup_result_temp = await Runner.run(
                redator_de_re_engajement_p_s_nurturing_fup,
                input=[
                    *conversation_history,
                    {
                        "role": "user",
            ...
            conversation_history.extend(
                [
                    item.to_input_item()
                    for item in redator_de_re_engajement_p_s_nurturing_fup_result_temp.new_items
                ]
            )

            redator_de_re_engajement_p_s_nurturing_fup_result = {
                "output_text": redator_de_re_engajement_p_s_nurturing_fup_result_temp.final_output.json(),
                "output_parsed": redator_de_re_engajement_p_s_nurturing_fup_result_temp.final_output.model_dump(),
            }
            end_result = {
                "titulo": redator_de_re_engajement_p_s_nurturing_fup_result[
                    "output_parsed"
                ]["titulo"],
                "corpo_html": redator_de_re_engajement_p_s_nurturing_fup_result[
                    "output_parsed"
                ]["corpo_html"],
            }
            return end_result
        else:
            return _fallback_output("Cadencia nao suportada ou ausente.")
